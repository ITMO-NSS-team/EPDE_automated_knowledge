import pandas as pd
import numpy as np
from drawbase.read_compile_df_tuned import read_csv


def melt_count_mae(count_ls):

    count_ls[0]["Name"] = "Classical"
    count_ls[1]["Name"] = "Modified, mf=2.4"
    count_ls[2]["Name"] = "Modified, tuned mf"
    if len(count_ls) == 4:
        count_ls[3]["Name"] = "Pysindy"
...
def melt_count_time(path, names, n_df=3):
    df_lsls = read_csv(path, names, n_df)
    for df in df_lsls[0]:
        df["Algorithm"] = "Classical"
    for df in df_lsls[1]:
        df["Algorithm"] = "Modified, mf=2.4"
    for df in df_lsls[2]:
        df["Algorithm"] = "Modified, tuned mf"
    if n_df == 4:
        for df in df_lsls[3]:
